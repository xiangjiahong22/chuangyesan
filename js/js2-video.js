...
(function () {
    ...
    if (index > 0) {
        nowpage = parseInt(url.substring(index + 1))
        $(".xiaoyuandian ul li").eq(nowpage).addClass("cur").siblings().removeClass("cur");
        goDown();
    }

    var lock = true;        //为了防止用户大量滚动滚轮设置的一个函数节流标识
    ...
    $(document).mousewheel(function (event, delta) {
        ...
        if (lock) {
            ...
            if (nowpage == 10 && delta == -1) {
                console.log("最底端下滚页面，footer值" + footer);
                $(".footer").stop().animate({ "height": 590 }, 500);   //动画显示footer
                $(".product-item:last").stop().animate({ "top": -590 }, 500);
                footer = true;  //改变信号量的值，表示此时footer已经打开。
                return;     //****这里的return是非常关键的！不再让我们的程序继续往下运行。此处不影响信号量
            }

            //判断是否到页面最底端，并且用户向上滚动，那么就应该隐藏footer
            if (nowpage == 10 && delta == 1 && footer) {
                console.log("最底端上滚页面，footer值" + footer);
                $(".footer").stop().animate({ "height": 0 }, 500);   //动画显示footer
                $(".product-item:last").stop().animate({ "top": 0 }, 500);
                footer = false;
                return;     //****这里的return是非常关键的！不再让我们的程序继续往下运行。此处不影响信号量
            }
    ...
    $(".xiaoyuandian ul li").click(function () {
        ...
        if ($(this).index() == 10) {
            $(".product-item:last").stop().animate({ "bottom": 0, 'top': 0 }, 500);
